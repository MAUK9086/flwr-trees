...
_CLF_KWARGS = dict(n_estimators=5, n_clients=3, random_state=42)
_REG_KWARGS = dict(n_estimators=5, n_clients=3, random_state=42)
...
# ---------------------------------------------------------------------------
# FederatedRandomForestClassifier — basic API
# ---------------------------------------------------------------------------


def test_classifier_fit_returns_self() -> None:
    X, y = _clf_data()
    clf = FederatedRandomForestClassifier(**_CLF_KWARGS)
    result = clf.fit(X, y)
...
def test_classifier_sets_estimators_after_fit() -> None:
    X, y = _clf_data()
    clf = FederatedRandomForestClassifier(n_estimators=5, n_clients=3, random_state=0)
    clf.fit(X, y)
    assert hasattr(clf, "estimators_")
    assert len(clf.estimators_) == 5 * 3
...
def test_classifier_predict_consistent_with_proba() -> None:
    """predict(X) must equal classes_[argmax(predict_proba(X))]."""
    X, y = _clf_data()
    clf = FederatedRandomForestClassifier(**_CLF_KWARGS)
    clf.fit(X, y)
    proba = clf.predict_proba(X)
...
def test_classifier_get_params_round_trips() -> None:
    clf = FederatedRandomForestClassifier(n_estimators=7, n_clients=4, random_state=1)
...
def test_classifier_reproducible_with_random_state() -> None:
    X, y = _clf_data()
    clf1 = FederatedRandomForestClassifier(n_estimators=5, n_clients=3, random_state=0)
    clf2 = FederatedRandomForestClassifier(n_estimators=5, n_clients=3, random_state=0)
    clf1.fit(X, y)
    clf2.fit(X, y)
...
def test_classifier_handles_n_clients_larger_than_n_samples() -> None:
    X, y = make_classification(n_samples=4, n_features=5, random_state=0)
    clf = FederatedRandomForestClassifier(n_estimators=2, n_clients=100, random_state=0)
    clf.fit(X, y)
    assert hasattr(clf, "estimators_")


# ---------------------------------------------------------------------------
# FederatedRandomForestRegressor — basic API
# ---------------------------------------------------------------------------


def test_regressor_fit_returns_self() -> None:
    X, y = _reg_data()
    reg = FederatedRandomForestRegressor(**_REG_KWARGS)
    result = reg.fit(X, y)
...
def test_regressor_sets_estimators_after_fit() -> None:
    X, y = _reg_data()
    reg = FederatedRandomForestRegressor(n_estimators=5, n_clients=3, random_state=0)
    reg.fit(X, y)
    assert len(reg.estimators_) == 5 * 3
...
def test_regressor_get_params_round_trips() -> None:
    reg = FederatedRandomForestRegressor(n_estimators=7, n_clients=4)
...
def test_regressor_reproducible_with_random_state() -> None:
    X, y = _reg_data()
    reg1 = FederatedRandomForestRegressor(n_estimators=5, n_clients=3, random_state=0)
    reg2 = FederatedRandomForestRegressor(n_estimators=5, n_clients=3, random_state=0)
    reg1.fit(X, y)
    reg2.fit(X, y)
